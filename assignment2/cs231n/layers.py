...
def affine_forward(x, w, b):
    ...
    X = x.reshape([-1, w.shape[0]]) #N D
    out = X @ w + b[np.newaxis,:]  #N M
...
def affine_backward(dout, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    X = x.reshape([-1, w.shape[0]]) #N D
    dw =  X.T @ dout # (N D).T * (N M) = D M
    
    db = dout.sum(axis=0) # M
    dx =  dout @ w.T  #  N M * (D M).T = N D
...
def batchnorm_backward(dout, cache):
    ...
    dfzu = -dfdz / cache['std']
    dfux = np.sum(dfzu,axis=0) / N

    dzdv = -0.5*(cache['var']**-1.5)*(cache['x']-cache['mean'])
    dfzv = dfdz * dzdv
    dvux = (2/N * (cache['x'] - cache['mean']))
    dvux += (-2/N * np.sum(cache['x'] - cache['mean'], axis=0))/N
    dfzvx = np.sum(dfzv,axis=0) * (dvux)
...
def batchnorm_backward_alt(dout, cache, axis=0):
    ...
    dgamma = np.sum(dout * cache['z'], axis=axis)
...
def layernorm_backward(dout, cache, axis):
    """
    ...
    dx, dgamma, dbeta = batchnorm_backward_alt(dout.T, cache, axis)
    dx = dx.T
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    reshaped_w = w.reshape(F, C*FH*FW)
    temp_out = np.zeros((C*FH*FW, outH*outW))  
    for n in range(N):
      counter_i = 0
      for i in range(0, x_pad.shape[2]-FH+1, stride):
        counter_j = 0
        for j in range(0, x_pad.shape[3]-FW+1,stride):
          out[n,:,counter_i,counter_j] = (w * x_pad[n,:,i:i+FH,j:j+FW][np.newaxis,:,:,:]).sum((1,2,3)).reshape([-1])+b            # F C FH FW * C, FH, FW  = F C FH FW ->sum -> F 1 1 1
          counter_j +=1
        counter_i += 1
...
def max_pool_forward_naive(x, pool_param):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, C, H, W = x.shape
    pool_H = pool_param.get('pool_height', 2)
    pool_W = pool_param.get('pool_width', 2)
    pool_S = pool_param.get('stride', 2)
    ...
    for n in range(N):
        for j in range(H_prime):
            for i in range(W_prime):
                out[n, :, j, i] = np.amax(x[n, :, j*pool_S:j*pool_S+pool_H, i*pool_S:i*pool_S+pool_W], axis=(-1, -2))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
